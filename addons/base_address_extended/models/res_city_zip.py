"""``CatalogPostalCode`` — contraparte de ``res.city.zip``
(Odoo ``base_address_extended``).

En Odoo 16+ el catálogo ciudad/CP (``res.city.zip``) vive en
``base_address_extended``. Aquí ``CatalogPostalCode`` cumple ese rol:
catálogo internacional de códigos postales (una fila por asentamiento
dentro de un CP), hoy con fuente SEPOMEX (MX). Movido state-only desde el
addon no-Odoo ``geo``; la tabla ``geo_catalog_postal_code`` no cambia.

Clave natural ``(country, postal_code, settlement_consecutive_id)`` con PK
surrogada (SOL-016, DEC-02, H-API-PARTY-01). ``Address`` (addons.users,
party model) referencia una fila concreta.
"""
