...
class AccountEdiDocument(models.Model):
    ...
    def __str__(self) -> str:
        return f'{self.edi_format_id}#{self.move_id}={self.state}'
    ...
    def name(self):
        ...
        return self.attachment.name if self.attachment_id else None

    @property
    def edi_format_name(self):
        """≙ ``edi_format_name`` (``related='edi_format_id.name'``)."""
        return self.edi_format.name
    ...
    def _compute_edi_content(self):
        ...
        if self.state not in ('to_send', 'to_cancel'):
            return b''
        move = self.move
        config_errors = self.edi_format._check_move_configuration(move)
        if config_errors:
            return base64.b64encode('\n'.join(config_errors).encode('UTF-8'))
        move_applicability = self.edi_format._get_move_applicability(move)
    ...
    def _process_job(cls, job):
        ...
        def _postprocess_post_edi_results(documents, edi_result):
            ...
            for document in documents:
                ...
                if move_result.get('attachment'):
                    ...
                    document.attachment = move_result['attachment']
        ...
        def _postprocess_cancel_edi_results(documents, edi_result):
            ...
            for document in documents:
                ...
                if move_result.get('success') is True:
                    ...
                    document.blocking_level = ''
                    document.save(update_fields=['state', 'error', 'attachment', 'blocking_level'])

                    if move.state == 'posted' and all(
                        d.state == 'cancelled' or not d.edi_format._needs_web_services()
        ...
        companies = {d.move.company_id for d in documents}
        ...
        moves = list({d.move for d in documents})
        if state == 'to_send':
    ...
    def _process_documents_no_web_services(cls, documents):
        ...
        batch = [d for d in documents if not d.edi_format._needs_web_services()]
    ...
    def _process_documents_web_services(cls, documents, job_count=None, with_commit=True):
        ...
        """
        batch = [d for d in documents if d.edi_format._needs_web_services()]
        ...
        for job in jobs_to_process:
            ...
            attachment_ids = {
                ...
                if d.attachment_id and not d.attachment.res_model and not d.attachment.res_id
            }
            try:
