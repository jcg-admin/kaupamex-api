...
# no-Celery pattern as apps.addons.backups) and the file is fetched later via a
...
_DOWNLOAD_SALT = 'apps.addons.reports.export-download'
...
def _run_export_job(job_pk: int) -> None:
    ...
    Runnable synchronously for tests (mirrors apps.addons.backups._run_backup).
    """
    try:
