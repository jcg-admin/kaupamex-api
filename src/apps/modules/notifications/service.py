"""
service.py — apps.modules.notifications
...
def notify_shipping_updated(order, user, tracking_number=None, event_description=None):
    ...
    Llamar desde apps.modules.logistics dentro de transaction.atomic().
    """
    if not user or not getattr(user, 'pk', None):
        return
...
def notify_return_processed(order, user, return_status, reason=None):
    ...
    Llamar desde apps.modules.returns dentro de transaction.atomic().
    """
    if not user or not getattr(user, 'pk', None):
        return
...
def notify_refund_processed(order, user, amount_refunded):
    ...
    Llamar desde apps.modules.payments dentro de transaction.atomic().
    """
    if not user or not getattr(user, 'pk', None):
        return
