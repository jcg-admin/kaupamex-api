"""
Models — apps.modules.users (modelo party, DEC-02/03, U-D puro)
...
IdentityUser: identidad + credenciales, 100% propio (NO hereda
    ...
    un ``Role`` de ``apps.platform.authz``, DEC-01=B).
...
class IdentityUserManager(models.Manager):
    ...
    def create_superuser(self, email, password=None, **extra_fields):
        ...
        en el seed de ``apps.platform.authz`` — U-D puro no tiene flag ``is_superuser``.
        """
        extra_fields.setdefault('is_active', True)
        return self._create_user(email, password, **extra_fields)
...
class IdentityUser(models.Model):
    ...
    # Company (L1) a la que pertenece el usuario. ``null`` = usuario cross-
    # company (operador de plataforma L0: superadmin/platform-operator) o sin
    # asignar. El gate L1-a del resolver (apps.platform.company) lo consume; aquí solo se
    # establece el vínculo de esquema. PROTECT: no se borra una Company con
    # usuarios vivos. Diseño: analisis-modelo-tenant-l1-foundation (DEC-T7).
...
class EmployeeProfile(TimeStampedModel):
    ...
    resuelve ``apps.platform.authz`` (roles/capacidades), NO este perfil."""
...
class MFADevice(TimeStampedModel):
    ...
    consumes it at login. See apps.modules.users.mfa (T-PLT-33).
    """
    TOTP = "totp"
