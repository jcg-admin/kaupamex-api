...
# no-Celery pattern as apps.modules.backups) and the file is fetched later via a
...
_DOWNLOAD_SALT = 'apps.modules.reports.export-download'
...
def _run_export_job(job_pk: int) -> None:
    ...
    Runnable synchronously for tests (mirrors apps.modules.backups._run_backup).
    """
    try:
