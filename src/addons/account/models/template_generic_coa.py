...
``account.chart.template`` con métodos decorados; aquí son funciones sueltas con
el mismo decorador, porque el registro no barre clases (ver la nota de
divergencia en ``chart_template.py``).
...
def get_generic_coa_template_data():
    """Los valores sueltos del plan — ≙ ``_get_generic_coa_template_data``.

    ``property_account_*`` no nombran una cuenta: nombran **el papel** que una
    cuenta cumple para la empresa. El identificador se resuelve contra las
    cuentas recién creadas, así que ``receivable`` acaba siendo la cuenta 1210
    *de esta empresa*.
    """
    return {
...
def get_generic_coa_res_company():
    """Lo que el plan escribe en la empresa — ≙ ``_get_generic_coa_res_company``.

    La referencia escribe aquí 18 claves; se conservan las que este puerto tiene
    campo donde poner. ``post_load_data`` descarta el resto en silencio y a
    propósito: la lista es de la referencia y describe campos que aún no
    existen aquí, así que abortar por uno accesorio impediría cargar el plan
    entero. Los que faltan entran solos cuando su campo llegue.
    """
    return {
