"""Views — addons.sale_subscription (consola L0 del operador Kaupamex).
...
from addons.sale_subscription.controllers.serializers import (
...
class CompanyViewSet(CapabilityRequiredMixin, ModelViewSet):
    ...
    queryset = ResCompany.objects.all().order_by('code')
    ...
    def suspend(self, request, pk=None):
        ...
        if company.status not in (ResCompany.Status.ACTIVE, ResCompany.Status.TRIAL):
            raise ValidationError({
                'detail': 'Solo un tenant activo o en prueba puede suspenderse.',
                'codigo_error': 'INVALID_STATUS_TRANSITION',
            })
        company.status = ResCompany.Status.SUSPENDED
    ...
    def reactivate(self, request, pk=None):
        company = self.get_object()
        if company.status != ResCompany.Status.SUSPENDED:
            raise ValidationError({
                'detail': 'Solo un tenant suspendido puede reactivarse.',
                'codigo_error': 'INVALID_STATUS_TRANSITION',
            })
        company.status = ResCompany.Status.ACTIVE
        company.save(update_fields=['status', 'updated_at'])
        return Response(CompanySerializer(company).data)
