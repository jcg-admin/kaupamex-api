"""Adopta los 6 modelos del framework de pagos en ``payment`` (hogar fiel Odoo).

State-only (``SeparateDatabaseAndState``): las tablas ``payments_*`` ya existen
(creadas por ``payments.0001_initial``); los modelos se re-alojan en ``payment``
(``payment.transaction``/``payment.token`` de Odoo). Solo cambia el ``app_label``
— sin DDL. La contraparte ``payments.0002`` los retira del estado de ``payments``.
"""
# Generated by Django 6.0.5 on 2026-07-21 00:17

import addons.payment.models.saved_card
        ...
        migrations.CreateModel(
            ...
            fields=[
                ...
                (
                    "verification_token",
                    models.CharField(
                        default=addons.payment.models.saved_card._make_verification_token,
