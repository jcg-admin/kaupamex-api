"""Controllers — reseñas de producto (``rating``, UC-REV-01..03).

Superficie restaurada del ex-addon ``reviews`` (retirado en
``api@36c3416``; los modelos ya vivían aquí desde el fold ``api@17ed279``).
En la referencia las reseñas viajan por el chatter sobre ``rating.rating``
(``odoo19c: portal_rating/controllers/{portal_chatter,portal_rating}.py``,
LGPL-3): mismo reparto — el rating es del módulo ``rating`` y la superficie
la expone el portal del sitio. Aquí el "portal" es esta API DRF; el
contrato JSON (stats + breakdown + moderación FIFO + votos + imágenes) es
el del producto (UC-REV), no un calco del QWeb de la fuente.

Public:
  GET  /api/v2/products/<product_id>/reviews/      Approved only + stats.
  POST /api/v2/products/<product_id>/reviews/      Create (auth required).

Admin:
  GET  /api/v2/admin/reviews/?status=PENDING_MODERATION  FIFO queue.
  POST /api/v2/admin/reviews/<id>/status/          approve/reject.

Spanish business error codes per DEC-DOC-006. Audit log per RNF-AUDIT-001.

Adaptación al catálogo plantilla/variante: el sujeto de la reseña es la
**variante** (``product.ProductProduct``) — el mismo eje que la FK
``Review.product`` ya usa.
"""
...
from addons.rating.controllers.serializers import (
...
class ProductReviewsView(APIView):
    ...
    def get(self, request, product_id):
        try:
            product = ProductProduct.objects.get(pk=product_id)
...
class ReviewStatusV2View(APIView):
    ...
    @extend_schema(
        summary='Moderar reseña (approve/reject)',
        tags=['admin-reviews'],
        request=inline_serializer('ReviewStatusRequest', {
            'action': serializers.ChoiceField(choices=['approve', 'reject']),
            'reason': serializers.CharField(required=False),
        }),
        responses={200: ReviewAdminSerializer,
                   400: error_response('INVALID_ACTION')},
    )
