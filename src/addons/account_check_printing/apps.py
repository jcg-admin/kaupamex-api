...
Cuatro extensiones sobre modelos AJENOS (≙ ``_inherit``) más una señal — todas
...
Las cuatro extensiones cuelgan un MÉTODO sobre un modelo de otro addon —
``chain_method`` (H-API-364), nunca la guarda ``hasattr``: si mañana otro
addon también extiende ``IrSequence.get_next_char`` o
...
class AccountCheckPrintingConfig(AppConfig):
    ...
    _EXTENSIONES = (
        ('addons.account_check_printing.models.ir_sequence',
    ...
    def ready(self):
        """Cuelga las tres extensiones y conecta la señal de auto-provisión.
