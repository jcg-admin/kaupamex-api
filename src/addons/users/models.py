"""
Models — addons.users (modelo party, DEC-02/03, U-D puro)
...
IdentityUser: identidad + credenciales, 100% propio (NO hereda
    ...
    un ``Role`` de ``addons.authz``, DEC-01=B).
...
class IdentityUserManager(models.Manager):
    ...
    def create_superuser(self, email, password=None, **extra_fields):
        ...
        en el seed de ``addons.authz`` — U-D puro no tiene flag ``is_superuser``.
        """
        extra_fields.setdefault('is_active', True)
        return self._create_user(email, password, **extra_fields)
...
class IdentityUser(models.Model):
    ...
    # asignar. El gate L1-a del resolver (addons.company) lo consume; aquí solo se
...
class EmployeeProfile(TimeStampedModel):
    ...
    resuelve ``addons.authz`` (roles/capacidades), NO este perfil."""
...
class MFADevice(TimeStampedModel):
    ...
    consumes it at login. See addons.users.mfa (T-PLT-33).
    """
    TOTP = "totp"
