...
# AuthzEvent (auditoría DEC-07) se movió a ``addons.authz_audit`` y ReauthSession
# (step-up DEC-12) a ``addons.authz_reauth`` (SOL-094 frente B, DEC-01): módulos
# de feature opcional al estilo Odoo (``account_audit_trail`` / ``auth_totp``).
# Sus tablas (``authz_event`` / ``authz_reauth_session``) quedan intactas
# (SeparateDatabaseAndState). Importar desde ``addons.authz_audit.models`` /
# ``addons.authz_reauth.models`` respectivamente.
