...
from apps.modules.logistics.models import CarrierRateCard, Courier
...
class TestShippingOffersAdminOnly:
    """La elección de paquetería es del ADMIN, no del comprador: el endpoint
    de la lista rankeada es admin-only (``logistics.manage``). El comprador
    nunca ve las paqueterías, sólo el costo final que resuelve el checkout."""

    def test_anonimo_no_autorizado(self, api_client, one_pkg, db):
        _card('fast', 'Fast')
        r = api_client.post(OFFERS_URL, one_pkg, format='json')
        assert r.status_code == 401

    def test_comprador_prohibido(self, auth_client, one_pkg, db):
        _card('fast', 'Fast')
        r = auth_client.post(OFFERS_URL, one_pkg, format='json')
        assert r.status_code == 403

    def test_admin_puede_cotizar(self, admin_client, one_pkg, db):
        _card('fast', 'Fast')
        r = admin_client.post(OFFERS_URL, one_pkg, format='json')
        assert r.status_code == 200, r.content
        assert 'offers' in r.data and 'ineligible' in r.data

    def test_paquetes_vacios_400(self, admin_client, db):
        r = admin_client.post(OFFERS_URL, {'packages': []}, format='json')
        assert r.status_code == 400

    def test_sin_packages_400(self, admin_client, db):
        r = admin_client.post(OFFERS_URL, {}, format='json')
        assert r.status_code == 400

    def test_dimension_no_positiva_400(self, admin_client, db):
        payload = {'packages': [{
            'length': '0', 'width': '10', 'height': '10',
            'weight': '1', 'value': '100',
        }]}
        r = admin_client.post(OFFERS_URL, payload, format='json')
        assert r.status_code == 400

    def test_ranking_por_costo(self, admin_client, one_pkg, db):
    ...
    def test_inelegible_reporta_motivo(self, admin_client, db):
    ...
    def test_solo_paqueterias_activas(self, admin_client, one_pkg, db):
        _card('activa', 'Activa')
