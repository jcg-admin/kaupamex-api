...
def _product(name, code):
    return make_product(name=name, price=Decimal('100.00'), default_code=code)
...
def test_bom_and_lines(db):
    ...
    bom = MrpBom.objects.create(
        product_tmpl=table.product_tmpl, product=table, product_qty=Decimal('1'))
    ...
    assert list(bom.bom_line_ids.values_list('product__default_code', flat=True)) == ['PAT-001', 'TAB-001']
...
def test_explode_kit_phantom_bom(db):
    ...
    bom = MrpBom.objects.create(
        product_tmpl=kit.product_tmpl, product=kit,
        type=MrpBom.TYPE_PHANTOM, product_qty=Decimal('1'))
    ...
    as_dict = {p.default_code: q for p, q in exploded}
