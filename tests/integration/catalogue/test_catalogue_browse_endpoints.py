...
required by the UI:
  ...
  GET  /api/v2/catalogue/search/?q=&category=&price_min=&price_max=&page=
...
class TestCategoryTree:
    ...
    def test_categorias_publicas(self, api_client, cat_browse, prod_browse, db):
        r = api_client.get('/api/v2/catalogue/categories/tree/')
        assert r.status_code == 200
...
class TestCatalogueSearchWrapper:
    ...
    def test_search_devuelve_normalized_query(
        ...
        r = api_client.get('/api/v2/catalogue/search/?q=  Yoruba  Sample  ')
        assert r.status_code == 200
        body = r.json()
    ...
    def test_search_persiste_history_para_auth(
        ...
        r = auth_client.get('/api/v2/catalogue/search/?q=yoruba')
        assert r.status_code == 200
