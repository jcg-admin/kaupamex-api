...
from apps.addons.payments.models import SavedCard
...
def test_save_card_creates_pending_and_sends_email(auth_client, user, mp_gw_cards):
    ...
    with patch('apps.addons.payments.gateways.mercadopago._get_sdk', return_value=mp_mock), \
         patch('apps.addons.payments.views.send_card_verification_email') as mock_email:
...
def test_save_card_no_customer_id_auto_creates(auth_client, user, mp_gw_cards):
    ...
    with patch('apps.addons.payments.gateways.mercadopago._get_sdk', return_value=mp_mock), \
         patch('apps.addons.payments.views.send_card_verification_email'):
        resp = auth_client.post(CARDS_URL, {
