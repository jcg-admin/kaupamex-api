...
Verifica la orquestación del adapter contra la ``ResCompany`` **real** (get_or_create
...
@mock.patch('addons.base.management.commands.company_create.provision_company_database')
def test_creates_row_and_provisions_db(mock_provision, db):
    ...
    company = ResCompany.objects.get(code='acme')
...
def test_second_run_is_idempotent(mock_provision, db):
    ...
    first_id = ResCompany.objects.get(code='globex').id

    out = StringIO()
    call_command('company_create', 'globex', stdout=out)

    assert ResCompany.objects.filter(code='globex').count() == 1
    assert ResCompany.objects.get(code='globex').id == first_id  # misma fila
    assert 'row_created=False' in out.getvalue()
    # default de nombre = código cuando no se pasa --name
    assert ResCompany.objects.get(code='globex').name == 'globex'
