...
class WebhookEvent(models.Model):
    """
    Registro de idempotencia para webhooks de pago. DEC-BC-04.

    UNIQUE(gateway, event_id, transmission_id) garantiza que el mismo
    webhook no se procese dos veces. La vista inserta este registro ANTES
    de procesar; si hay IntegrityError el evento ya fue procesado.
    """
    gateway         = models.CharField(
        max_length=20, choices=Payment.GATEWAYS, db_index=True,
    )
    event_id        = models.CharField(max_length=200)
    transmission_id = models.CharField(max_length=200, blank=True, default='')
    raw_body        = models.TextField()
    ...
    def __str__(self):
        return f'{self.gateway} {self.event_id} {self.transmission_id}'
