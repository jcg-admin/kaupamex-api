...
from .models import Category, Product, ProductDiscount, ProductImage, SearchHistory
...
class ProductListSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = [
            ...
            'main_image', 'discount',
            'variants_available', 'is_featured',
    ...
    def get_main_image(self, obj) -> str | None:
        """UC-CAT-01: cover image URL, or first by order if no cover."""
        img = (
            obj.images.filter(is_cover=True).first()
            or obj.images.order_by('order').first()
        )
        return img.image.url if img else None

    def get_discount(self, obj) -> dict | None:
        """Active ProductDiscount (CURRENT status) for this product."""
        now = timezone.now()
        d = (
            obj.discounts
...
class ProductDetailSerializer(serializers.ModelSerializer):
    ...
    def get_images(self, obj) -> list:
        """UC-CAT-02 / DEC-BC-17 17b: all product images ordered by position."""
        return ProductImageSerializer(
            obj.images.order_by('order'), many=True
        ).data

    def get_discount(self, obj) -> dict | None:
        """UC-CAT-02 / DEC-BC-17 17a: active ProductDiscount (CURRENT) for this product."""
        now = timezone.now()
        d = (
            obj.discounts
            .filter(is_active=True, valid_from__lte=now)
    ...
    def get_reviews_summary(self, obj) -> dict:
        """
        UC-CAT-02 / UC-REV-01: aggregate over APPROVED reviews only.
        Returns avg_rating (1 decimal) and total_count.
        Only buyers (order FK present on Review) can write reviews —
        enforced at create time in the reviews endpoint.
        """
        result = (
            obj.reviews
            .filter(status=Review.STATUS_APPROVED)
            .aggregate(avg=Avg('rating'), count=Count('id'))
        )
        return {
            'average_rating': round(result['avg'], 1) if result['avg'] else None,
            'total_count': result['count'],
        }

    def get_questions_count(self, obj) -> int:
        """UC-CAT-02 / UC-QST-01: publicly visible questions (ANSWERED, non-empty body)."""
        return (
            obj.questions
            .filter(status=QuestionStatus.ANSWERED, answer_body__gt='')
            .count()
        )
